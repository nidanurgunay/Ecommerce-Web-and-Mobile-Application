...
class womenHeels extends Component {
  constructor() {
    super();
    this.state = {
      products: [],
  ...
  componentDidMount = async () => {
    try {
      const response = await tracker.get("product/gender/female/category/Heels");
    ...
    if (this.state.products.length !== 0) {
      ...
      for (var i = 0; i < this.state.products.length; i++) {
        ...
        productitem = (
            ...
            <CardLink
              onClick={() => {
                this.props.history.push("/product/" + id + '/wh/' + coni);
              }}
              style={{ color: "black" }}
            >
              <AliceCarousel
  ...
  handleOrder = async (e,price,val) =>{
    console.log("orderr")
    try {
      const response = await tracker.get("product/gender/female/category/Heels/"+price+"/"+val);
      const arrayproduct = response.data;
  ...
  render() {
    return (
      <>
        <Container style={{ width: "auto", textAlign: "-webkit-center", height:"auto" }}>
          <div className="card" style={{ width: "auto", marginRight: "1vh", marginLeft: "1vh", maxWidth: "initial",height:"auto"  }}>
            <h1 className="mt-5">Woman Heels</h1>
            <Navbar collapseOnSelect expand="lg" bg="light" variant="light" {...this.props}>
              <NavDropdown title="Order by" id="collasible-nav-dropdown">
                ...
                <NavDropdown.Item onSelect={(e) => this.handleOrder(e,"price","-1")}>Price (Descending)</NavDropdown.Item>
                    ...
                    <Row style={{ margin: "5vh", justifyContent: "space-around" , height:"auto" }}>{this.handleProducts()}</Row>
          </div>
        </Container>
      </>
    );
